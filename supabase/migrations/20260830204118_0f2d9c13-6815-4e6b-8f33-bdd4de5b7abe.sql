...
ADD COLUMN IF NOT EXISTS processamento_meta jsonb NOT NULL DEFAULT '{}'::jsonb;

COMMENT ON COLUMN public.documentos.processamento_meta IS
'Diagnóstico persistente e progresso retomável da leitura, indexação e interpretação do documento.';
...
BEGIN
  ...
  IF NOT public.pode_no_condominio(auth.uid(), p_condominio_id, 'editar')
     AND NOT public.is_any_admin(auth.uid()) THEN
  ...
  LOOP
    ...
    IF v_numero IS NULL THEN
      RAISE EXCEPTION 'Unidade sem número';
    END IF;
    ...
    IF v_id IS NULL THEN
      INSERT INTO public.unidades (
        condominio_id, bloco, numero, tipo, fracao_ideal, area_m2, vagas_garagem
      ) VALUES (
        p_condominio_id, v_bloco, v_numero, v_tipo, v_fracao, v_area, v_vagas
      );
      v_criadas := v_criadas + 1;
    ELSIF p_estrategia = 'substituir' THEN
      UPDATE public.unidades
      SET tipo = v_tipo,
          fracao_ideal = v_fracao,
          area_m2 = v_area,
          vagas_garagem = v_vagas,
          updated_at = now()
      WHERE id = v_id;
      v_atualizadas := v_atualizadas + 1;
    ELSIF p_estrategia = 'preencher' THEN
      UPDATE public.unidades
      SET fracao_ideal = CASE WHEN fracao_ideal IS NULL THEN v_fracao ELSE fracao_ideal END,
