  ...
  .handler(async ({ data, context }) => {
    ...
    for (const d of destinatarios) {
      const html = compilarEmailConvocacao({
        ...
        dataExtenso: new Date(conv.assembleia.data_inicio).toLocaleDateString('pt-BR', { dateStyle: 'long' }),
        horario: new Date(conv.assembleia.data_inicio).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }),
