...
function OnboardingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0A1220] text-[#F4F5F7] px-4 py-10">
      <div className="max-w-3xl mx-auto">
        <div className="flex justify-center mb-8"><Logo variant="inverted" size="md" /></div>

        <div className="flex items-center justify-between mb-10 border-b border-[#1F2937] pb-6">
        ...
        {step === 1 && (
          <div className="rounded-xl border border-[#1F2937] bg-[#0F1929] p-8">
            <h1 className="text-2xl font-bold">Confirme seus dados</h1>
            <p className="text-slate-400 mt-1 text-sm">Você pode alterar esses dados depois em seu perfil.</p>
            <div className="mt-6 grid sm:grid-cols-2 gap-4">
              <div className="space-y-1.5 sm:col-span-2">
                <Label className="text-xs uppercase text-slate-400">Nome completo</Label>
                <Input value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })}
                  className="bg-[#152033] border-[#1F2937] text-white" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs uppercase text-slate-400">Telefone</Label>
                <Input value={form.telefone} onChange={(e) => setForm({ ...form, telefone: e.target.value })}
                  className="bg-[#152033] border-[#1F2937] text-white" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs uppercase text-slate-400">E-mail</Label>
                <Input value={profile?.email ?? ""} disabled className="bg-[#152033] border-[#1F2937] text-slate-500" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs uppercase text-slate-400">Tipo de pessoa</Label>
                <div className="grid grid-cols-2 gap-2">
                  {(["pf","pj"] as const).map((t) => (
                    <button key={t} type="button" onClick={() => setForm({ ...form, tipo_pessoa: t })}
                      className={`rounded-md border px-3 py-2 text-sm font-medium ${
                        form.tipo_pessoa === t ? "border-emerald-500 bg-emerald-500/10 text-emerald-400"
                          : "border-[#1F2937] text-slate-300"}`}>
              ...
              <div className="space-y-1.5">
                <Label className="text-xs uppercase text-slate-400">{form.tipo_pessoa === "pf" ? "CPF" : "CNPJ"}</Label>
                <Input value={form.cpf_cnpj} onChange={(e) => setForm({ ...form, cpf_cnpj: e.target.value })}
                  className="bg-[#152033] border-[#1F2937] text-white" />
              </div>
              {form.tipo_pessoa === "pj" && (
                <div className="space-y-1.5 sm:col-span-2">
                  <Label className="text-xs uppercase text-slate-400">Razão Social</Label>
            ...
            <div className="mt-8 flex justify-end">
              <Button onClick={nextFromStep1} disabled={busy}
                className="bg-emerald-500 hover:bg-emerald-600 text-white font-semibold">Continuar</Button>
            </div>
          </div>
        )}
                ...
                return (
                  <div key={p.id}
                    className={`relative rounded-xl border p-6 flex flex-col ${
                      destaque ? "border-emerald-500 bg-[#0F1929]" : "border-[#1F2937] bg-[#0F1929]"
                    }`}>
                    {destaque && (
                      <span className="absolute -top-2.5 left-6 rounded-full bg-emerald-500 text-white text-[10px] font-bold uppercase tracking-wide px-2 py-0.5">
                        Mais popular
                      </span>
                    )}
                    ...
                    <p className="mt-2 text-xs text-slate-400">{p.descricao}</p>
                    ...
                    <Button onClick={() => escolherPlano(p)} disabled={busy}
                      className={`mt-5 w-full ${destaque ? "bg-emerald-500 hover:bg-emerald-600" : "bg-white text-[#0A1220] hover:bg-slate-200"}`}>
        ...
        {step === 3 && (
          <div className="rounded-xl border border-[#1F2937] bg-[#0F1929] p-8">
            <div className="flex items-center gap-2"><Building2 className="h-5 w-5 text-emerald-400" />
            ...
            <div className="mt-6 grid sm:grid-cols-2 gap-4">
              <div className="space-y-1.5 sm:col-span-2">
                <Label className="text-xs uppercase text-slate-400">Nome do condomínio</Label>
                <Input value={condo.nome} onChange={(e) => setCondo({ ...condo, nome: e.target.value })}
                  className="bg-[#152033] border-[#1F2937] text-white" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs uppercase text-slate-400">CNPJ (opcional)</Label>
                <Input value={condo.cnpj} onChange={(e) => setCondo({ ...condo, cnpj: e.target.value })}
                  className="bg-[#152033] border-[#1F2937] text-white" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs uppercase text-slate-400">UF</Label>
                <Input maxLength={2} value={condo.uf} onChange={(e) => setCondo({ ...condo, uf: e.target.value.toUpperCase() })}
                  className="bg-[#152033] border-[#1F2937] text-white" />
              </div>
              <div className="space-y-1.5 sm:col-span-2">
                <Label className="text-xs uppercase text-slate-400">Endereço</Label>
                <Input value={condo.endereco} onChange={(e) => setCondo({ ...condo, endereco: e.target.value })}
                  className="bg-[#152033] border-[#1F2937] text-white" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs uppercase text-slate-400">Número de unidades</Label>
      ...
      <Dialog open={contatoOpen} onOpenChange={setContatoOpen}>
        <DialogContent className="bg-[#0F1929] border-[#1F2937] text-white">
          <DialogHeader>
            <DialogTitle className="text-white flex items-center gap-2">
              <MessageSquare className="h-5 w-5 text-emerald-400" /> Falar com consultor
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3 mt-2">
            ...
            <div className="space-y-1.5">
              <Label className="text-xs uppercase text-slate-400">Mensagem</Label>
              <textarea value={contato.mensagem} onChange={(e) => setContato({ ...contato, mensagem: e.target.value })}
                rows={4} className="w-full rounded-md bg-[#152033] border border-[#1F2937] text-white p-2 text-sm" />
            </div>
            <Button onClick={enviarContatoForm} disabled={busy}
              className="w-full bg-emerald-500 hover:bg-emerald-600 text-white">Enviar</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
